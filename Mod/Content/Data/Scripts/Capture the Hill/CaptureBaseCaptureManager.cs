...
namespace CaptureTheHill.Content.Data.Scripts.Capture_the_Hill
{
    ...
    {
        public static void UpdateBaseCaptureProgress(Dictionary<string, List<CaptureBaseData>> basesPerPlanet)
        {
            ...
            foreach (var cp in captureBases)
            {
                    ...
                    cp.FightMode == CaptureBaseFightMode.Defending && cp.CaptureProgress > 0)
                {
                    ...
                    if (cp.CaptureProgress == 0)
                    {
                        ...
                        CthLogger.Info($"{cp.BaseName} is now neutral");
                        continue;
                    }
                ...
                {
                    if (cp.CurrentDominatingFaction != cp.PreviousDominatingFaction &&
                    ...
                    if (cp.CaptureProgress >= baseCaptureTime)
                    {
                        ...
                        CthLogger.Info($"{cp.BaseName} has been captured by faction {cp.CurrentOwningFaction}");
