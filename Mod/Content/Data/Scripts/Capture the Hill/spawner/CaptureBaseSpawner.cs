...
namespace CaptureTheHill.Content.Data.Scripts.Capture_the_Hill.spawner
{
    public static class CaptureBaseSpawner
    {
        public static void CheckAndCreateBasesIfNeeded(HashSet<IMyEntity> planets)
        {
            ...
            {
                CthLogger.Info("Keine Planeten gefunden, Capture Base wird nicht erstellt.");
                return;
            }
            ...
            foreach (var planetEntity in planets)
            {
                ...
                if (expectedPlanetBaseCount >= 1 && !basesOfPlanet.Any(e => e.Name.EndsWith("ground")))
                {
                    ...
                    {
                        CthLogger.Error($"Found no position for Capture Base Ground on {planet.Name}.");
                    }
                }
                ...
                if (expectedPlanetBaseCount >= 2 && !basesOfPlanet.Any(e => e.Name.EndsWith("atmosphere")))
                {
                    ...
                    {
                        CthLogger.Error($"Found no position for Capture Base Atmosphere on {planet.Name}.");
                    }
                }
                ...
                if (expectedPlanetBaseCount == 3 && !basesOfPlanet.Any(e => e.Name.EndsWith("space")))
                {
                    ...
                    {
                        CthLogger.Error($"Found no position for Capture Base Space on {planet.Name}.");
                    }
                }
            }
        }
            ...
            string prefabSubtypeId)
        {
            ...
            {
                CthLogger.Error($"Invalid parameters for creating capture base on [{planetName}] at {position}.");
                return;
            }
            ...
            List<IMyCubeGrid> spawnedGrids)
        {
            foreach (var spawnedGrid in spawnedGrids)
            {
                ...
                {
                    CthLogger.Error($"Could not cast grid with name {spawnedGrid.Name} to MyCubeGrid.");
                    continue;
                }
