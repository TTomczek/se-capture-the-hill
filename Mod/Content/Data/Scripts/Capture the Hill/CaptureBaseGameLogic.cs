...
namespace CaptureTheHill.Content.Data.Scripts.Capture_the_Hill
{
    ...
    public class CaptureBaseGameLogic : MyGameLogicComponent
    {
        ...
        public override void Init(MyObjectBuilder_EntityBase captureBaseGrid)
        {
            ...
            {
                CthLogger.Info(
                    $"CaptureBaseData is null for {CaptureBaseGrid.Name} in Init, will retry in next frame.");
        ...
        private void HandleBaseDiscovery(List<long> playerIdsInSphere)
        {
            if (ModConfiguration.Instance.BroadcastBaseDiscoveryToFaction)
            {
                ...
                foreach (var playerId in playerIdsInSphere)
                {
                    ...
                    {
                        CthLogger.Info($"Player {playerId} has no faction, sending individual GPS");
            ...
            CthLogger.Debug($"Adding {playerIdsInSphere.Count} players to base discovery for {CaptureBaseGrid.Name}");
        }
        ...
        private void CheckCapturing()
        {
            ...
            {
                CthLogger.Debug($"{CaptureBaseGrid.DisplayName} is being captured by faction {dominatingFaction}");
        ...
        private long GetDominatingFaction(List<MyCubeGrid> vehiclesInSphere)
        {
            ...
            foreach (var vehicle in vehiclesInSphere)
            {
                ...
                CthLogger.Debug($"Vehicle {vehicle.DisplayName} is owned by: {ownerId}");
                if (ownerId == 0)
                {
                    continue;
                }
                ...
                CthLogger.Debug($"Owner {ownerId} faction: {(playerFaction != null ? playerFaction.Name : "None")}");
                if (playerFaction == null)
                {
            ...
            CthLogger.Debug($"Faction counts in {CaptureBaseGrid.DisplayName}: " +
                            string.Join(", ", factionCount.Select(kv => $"{kv.Key}: {kv.Value}")));

            if (factionCount.Count == 0)
            {
                CthLogger.Debug($"No factions present in {CaptureBaseGrid.DisplayName}");
                return 0;
            }
