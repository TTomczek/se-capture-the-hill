...
namespace CaptureTheHill.Content.Data.Scripts.Capture_the_Hill.state
{
    ...
    public class CaptureBaseData
    {
        ...
        [ProtoMember(10)] public long PreviousDominatingFaction = 0;
        ...
        {
            return
                $"{nameof(PlanetName)}: {PlanetName}, {nameof(BaseName)}: {BaseName}, {nameof(BaseDisplayName)}: {BaseDisplayName}, {nameof(CaptureBaseType)}: {CaptureBaseType}, {nameof(CurrentOwningFaction)}: {CurrentOwningFaction}, {nameof(CurrentDominatingFaction)}: {CurrentDominatingFaction}, {nameof(CaptureProgress)}: {CaptureProgress}, {nameof(FightMode)}: {FightMode}, {nameof(LastNotifiedFaction)}: {LastNotifiedFaction}, {nameof(PreviousDominatingFaction)}: {PreviousDominatingFaction}";
        }
    }
}